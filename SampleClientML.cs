...
namespace SampleClientML
{
    public class SampleClientML
    {
        ...
        private static string mStrLocalIP = "192.168.31.103";   // Local IP address (string)
        private static string mStrServerIP = "192.168.31.133";  // Server IP address (string)
        private static NatNetML.ConnectionType mConnectionType = ConnectionType.Multicast; // Multicast or Unicast mode
        //ming
        private static IPEndPoint drone_ep = new IPEndPoint(IPAddress.Parse("192.168.4.1"), 14550); //assume your drone is connected to PC running SampleClientML through wifi and its ip is 192.168.99.1
        private static Socket mavSock = new Socket(AddressFamily.InterNetwork, SocketType.Dgram, ProtocolType.Udp);
        private static Stopwatch stopwatch;
        private static MAVLink.MavlinkParse mavlinkParse = new MAVLink.MavlinkParse();//ming
        //ming
